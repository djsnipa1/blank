  ...
  */
	...
	const sketch: Sketch = (p5) => {
		...
		p5.draw = () => {
			...
			for (let i = marginX / 2 + slotSize / 2; i < p5.width - marginX / 2; i += slotSize) {
				for (let j = marginY / 2 + slotSize / 2; j < p5.height - marginY / 2; j += slotSize) {
					// code here
					p5.fill(255);
...
<input bind:value={slotSize} placeholder="100" class="absolute top-10 left-10 z-0" />
