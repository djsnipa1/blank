...
function confetti(container, options = {}) {
  ...
  let particles = [];
  let nodes = [];
  const calc_rotation_transform = () =>
    math_round(random() * (POSSIBLE_ROTATION_TRANSFORMS - 1));
  const get_is_circle = (particle_shape, rotation_transform) =>
  ...
  function confetti_styles(node, degree) {
    ...
    set_css_var(
      '--x4',
      round(abs(map_range(abs(rotate(degree, 90) - 180), 0, 180, -1, 1)), 4)
    );
    ...
    set_css_var(
      '--y4',
      round(max(map_range(abs(degree - 180), 0, 180, force, -force), 0), 4)
    );
    set_css_var(
      // --width
      '--w',
      (is_circle ? particleSize : math_round(random() * 4) + particleSize / 2) +
        'px'
    );
    set_css_var(
      // --height
      '--h',
      (is_circle ? particleSize : math_round(random() * 2) + particleSize) +
        'px'
    );
  ...
  return {
    update(new_options) {
      const new_particle_count =
        new_options.particleCount ?? DEFAULT_PARTICLE_COUNT;
      const new_particle_shape =
        new_options.particleShape ?? DEFAULT_PARTICLE_SHAPE;
      const new_particle_size =
        new_options.particleSize ?? DEFAULT_PARTICLE_SIZE;
      const new_particle_class =
        new_options.particleClass ?? DEFAULT_PARTICLE_CLASS;
      ...
      const new_destroy_after_done =
        new_options.destroyAfterDone ?? DEFAULT_DESTROY_AFTER_DONE;
        ...
        for (const [i, { color }] of object_entries(particles)) {
          ...
          if (new_particle_shape !== particleShape) {
            node.style.setProperty(
              // --border-radius
              '--br',
              get_is_circle(new_particle_shape, calc_rotation_transform())
                ? '50%'
                : '0'
            );
          }
...
var create_particles = (count, colors) =>
  Array.from({ length: count }, (_, i) => ({
    color: colors[i % colors.length],
    degree: (i * 360) / count
  }));
var round = (num, precision = 2) =>
  math_round((num + Number.EPSILON) * 10 ** precision) / 10 ** precision;
var map_range = (value, x1, y1, x2, y2) =>
  ((value - x1) * (y2 - x2)) / (y1 - x1) + x2;
var rotate = (degree, amount) =>
...
var should_be_circle = (rotation_transform) =>
  rotation_transform !== 1 && coin_flip();
