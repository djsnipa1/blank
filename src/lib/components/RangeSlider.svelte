...
<input
  ...
  max="100"
  value="40"
  class="w-2/3 h-[20px] bg-pink-500 border-none"
/>
...
*/
  ...
  $track-h: 1.125rem;
  $track-r: 0.5 * $track-h;
  $inner-s: inset 0 2px 2px rgba(#f7f7f7, 0.2);
  $outer-s: 0 1px 1px #1a1a1a;
  $track-c: #303030;
  ...
  $thumb-d: $track-h;
  $thumb-c: #8a8a8a, #e0e0e0, #909090, #f1f2f1, #949392, #cfcfcf;
  $thumb-s: ();
  ...
  @for $i from 1 to 13 {
    $thumb-s:
      $thumb-s,
      0 $i * 1px 3px rgba(#000, 0.05);
  }

  @mixin track($f: 0) {
    @if $f > 0 {
      --dim: calc(var(--js) * 0.01 * var(--val) * 100%);
      margin: $progr-m;
      background: $progr-g 0 / var(--dim) no-repeat;
    } @else {
      width: calc(100% - 2 *#{$progr-m});
    }
  ...
  @mixin thumb($o: 0) {
    ...
    box-shadow:
      $thumb-s,
      inset 0 0 0 2px rgba(#ededed, 0.2);
    background: conic-gradient($thumb-c, nth($thumb-c, 1));
    cursor: ew-resize;
  }
  ...
  * {
  ...
  form {
    ...
    > * {
      ...
      background: linear-gradient(to right top, #474747, #202020) padding-box;
    }
  ...
  [type='range'] {
	...
	* can coincide with input */
    ...
    &::-moz-range-progress {
      ...
      border-right-width: calc(0.01 * var(--val) *#{$progr-m});
      height: $progr-h;
      border-radius: $track-r;
      box-shadow: $inner-s;
